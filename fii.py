...
def scraping():
    ...
    for row in rows:
        ...
        temp = {'fundo': cols[0].text,
                ...
                #'dividendoYield': cols[5].text.replace('%','').replace(',','.').strip(),
                ...
                #'vacanciaFisica': cols[23].text.replace('%', '').replace(',','.'),
                #'vacanciaFinanceira': cols[24].text.replace('%', '').replace(',','.'),
